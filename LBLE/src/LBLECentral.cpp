...
uint32_t LBLEAdvertisements::getRawManufacturerData(uint8_t* dstBuf, uint32_t bufLen)
{
    return  getAdvDataWithTypeFromPayload(0xFF, dstBuf, bufLen, adv_data);
}

uint32_t LBLEAdvertisements::getRawAdvertisementData(uint8_t* dstBuf, uint32_t bufLen)
{
    if(bufLen < adv_data.data_length) {
        return -1; // Buffer is too small
    }
    
    memcpy(dstBuf, adv_data.data, adv_data.data_length);
    
    return adv_data.data_length; 
}
...
void LBLECentralClass::scan()
{
    ...
    // m_peripherals_found.clear(); // Clear peripheral set 
...
uint32_t LBLECentralClass::getRawManufacturerData(int index, uint8_t* dstBuf, uint32_t bufLen)
{
    ...
    return parser.getRawManufacturerData(dstBuf, bufLen);       
}

uint32_t LBLECentralClass::getRawAdvertisementData(int index, uint8_t* dstBuf, uint32_t bufLen)
{
    LBLEAdvertisements parser(m_peripherals_found[index]);
    
    return parser.getRawAdvertisementData(dstBuf, bufLen);    
}

void LBLECentralClass::clear(void)
{
